...
# Même fonction pour sortir la proportion de long mais à pour les données de Bissolfzik visuel
def extract_long_court_visuel(all_trie, filename):
    final_list = []

    # Parcours du fichier
    for row in all_trie:
        # Row
        # 32 : Type sujet : 1 court Entree, 2 court Control
        # 36 : 1 pour Biss1, 2 pour intermede, 3 pour Biss2
        # 74 : 1 pour ENTER, 2 pour CONTROL
        # 46 : procédure : APPRDUREE, TESTDUR, BISS1, BISS2
        # 55 : durée stimulus

        # On garde toutes les lignes avec les réponses longues et la ligne vide de l'intermède
        if row[46] == 'BISS1' or row[46] == 'BISS2':
            if row[32] != row[74]:  # on ne garde que les réponses longues
                final_list.append(row)
        #
        # if row[36] == '2':  # et la ligne vide pour l'intermède
        #     final_list.append(row)
    ...
    # Première ligne du fichier final
    with open(filename[:-14]+'Long_court.csv',
              'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(head)
    ...
    biss2 = False # Marqueur pour savoir si on est passé à la deuxième bissection

    for i in range(1, len(final_list)):
        if int(final_list[i][1]) == int(liste_sujets[j]):  # Si je suis encore au même sujet

            # Si j'arrive à la deuxième bissection, j'enregistre les valeurs de la première
            if final_list[i][46] == 'BISS2' and biss2 == False:
                compt_duree1 = compt_duree2
                compt_duree2 = [0, 0, 0, 0, 0, 0, 0]
                biss2 = True

            # Comptage
            if final_list[i][55] == '208':
                compt_duree2[0] += 1
            if final_list[i][55] == '272':
                compt_duree2[1] += 1
            if final_list[i][55] == '336':
                compt_duree2[2] += 1
            if final_list[i][55] == '400':
                compt_duree2[3] += 1
            if final_list[i][55] == '464':
                compt_duree2[4] += 1
            if final_list[i][55] == '528':
                compt_duree2[5] += 1
            if final_list[i][55] == '592':
                compt_duree2[6] += 1
        ...
        # Si j'ai fini les données d'un sujet
            ...
            # J'ajoute ces données à mon fichier
            with open(filename[:-14]+'Long_court.csv','a', newline='') as f:
                writer = csv.writer(f)
                writer.writerows(res)
